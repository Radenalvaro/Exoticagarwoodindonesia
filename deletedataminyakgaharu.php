...
if (isset($_GET['id']) && is_numeric($_GET['id'])) {
    ...
    try {
        ...
        $stmt = $pdo->prepare("SELECT image_url FROM products WHERE id = ? AND category = 'minyak_gaharu'");
        $stmt->execute([$product_id]);
        $product = $stmt->fetch();
        
        if ($product) {
            // Delete the product from database
            $stmt = $pdo->prepare("DELETE FROM products WHERE id = ? AND category = 'minyak_gaharu'");
            ...
            if ($result) {
                ...
                $_SESSION['success_message'] = 'Produk minyak gaharu berhasil dihapus!';
            } else {
